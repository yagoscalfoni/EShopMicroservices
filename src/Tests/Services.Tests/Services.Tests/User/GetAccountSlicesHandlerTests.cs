...
public class GetAccountSlicesHandlerTests
{
    [Fact]
    public async Task Handle_ShouldReturnEachSliceWithMinimalProjection()
    {
        ...
        var overviewResult = await new GetAccountOverviewHandler(context)
            .Handle(new GetAccountOverviewQuery(userId.Value), CancellationToken.None);
        var profileResult = await new GetProfileDetailsHandler(context)
            .Handle(new GetProfileDetailsQuery(userId.Value), CancellationToken.None);
        var addressesResult = await new GetAddressesHandler(context)
            .Handle(new GetAddressesQuery(userId.Value), CancellationToken.None);
        var paymentsResult = await new GetPaymentMethodsHandler(context)
            .Handle(new GetPaymentMethodsQuery(userId.Value), CancellationToken.None);
        var supportResult = await new GetSupportTicketsHandler(context)
            .Handle(new GetSupportTicketsQuery(userId.Value), CancellationToken.None);

        overviewResult.Overview.Benefits.Should().ContainSingle().And.Contain("Frete grátis");
        profileResult.Profile.SecurityRecommendations.Should().Contain("Ative 2FA");
        addressesResult.Addresses.Should().ContainSingle(a => a.Default);
        paymentsResult.PaymentMethods.Should().ContainSingle(p => p.Preferred);
        supportResult.SupportTickets.Should().Contain(ticket => ticket.Id == "SUP-1000");
    }
}
